...
export default function Navbar() {
  ...
  return (
    <header className="bg-primary border-b sticky top-0 z-50">
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
  <div className="md:hidden fixed top-0 right-0 h-1/3 w-1/2 bg-primary bg-opacity-90 backdrop-blur-sm p-6 flex flex-col gap-6 text-right font-bold text-sm uppercase text-white rounded-bl-lg items-end z-50">
    {menu.map((item) => (
      <Link
        key={item.label}
        to={item.to}
